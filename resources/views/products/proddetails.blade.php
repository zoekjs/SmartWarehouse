      ...
      <div class="card header bg-dark text-light d-flex">
        <div class="row">
          ...
          <div class="col-md 3 d-flex justify-content-end mr-4">
            <form action="{{action('IngressDetailController@update')}}" method="post"">
              @csrf
              ...
              <input class="form-control" type="text" name="id_ingress" id="" hidden value="{{$id_ingress}}">
            </form>
          </div>
        </div>
      </div>
      <div class="card-body">
        <form method="post" id="podForm" action="{{action('IngressDetailController@store')}}">
          @csrf
          <input type="text" hidden name="rut_user" id="rut_user" value="{{Auth()->user()->rut_user}}">
          <div class="row">
            <div class="col-md-12">
                 ...
                 <table id='table_details' class="table table-bordered table-condensed table-sm bg-white">
                    ...
                    <tbody>
                      @foreach($details as $detail)
                        <tr>
                          <td class='text-center' hidden>{{$detail->id_ingress}}</td>
