<template>
        <div>
            <div class="d-flex mb-3 col-12">
                <b-modal id="modal-add-product-1" centered ref="createProductModal" title="Añadir producto">
                    ...
                    <template #modal-footer="{ aceptar, cancel}">
                        <b-button size="sm" variant="success" :disabled="disabled" @click="createProduct()">
                            Aceptar
                        </b-button>
                        <b-button size="sm" variant="danger" @click="clearProductsForm()">
                            Cancelar
                        </b-button>
                    </template>
                </b-modal>
                <!-- MODAL EDIT PRODUCTS -->
                <b-modal v-model="show" centered ref="createProductModal" title="Editar producto" @hide="clearProductsForm()">
                    <div class="form-group">
                        <div class="input-field col-sm-12">
                            <label for="id_productEdit">Código de producto</label>
                            <input class="form-control" type="text" v-model="idProduct" id="id_productEdit" readonly>
                        </div>
                    </div>
                    <div class="form-group">
                        <div class="input-field col-sm-12">
                            <label for="nameEdit">Nombre producto</label>
                            <input class="form-control" type="text" v-model="name" id="nameEdit" required>
                        </div>
                    </div>
                    <div class="form-group">
                        <div class="input-field col-sm-12">
                            <label for="descriptionEdit">Descripción</label>
                            <textarea id="descriptionEdit" v-model="description" class="form-control"></textarea>
                        </div>
                    </div>
                    <div class="row">
                        <div class="input-field col-sm-6">
                            <label for="quantityEdit">Cantidad</label>
                            <input class="form-control" type="text" v-model="quantity" id="quantityEdit" required>
                        </div>
                        <div class="form-group col-sm-6">
                            <label for="unit_priceEdit">Precio Unitario</label>
                            <input class="form-control" type="text" v-model="unitPrice" id="unit_priceEdit" required="true">
                        </div>
                    </div>
                    <template #modal-footer="{ aceptar, cancel}">
                        <b-button size="sm" :disabled="disabled" variant="success" @click="updateProduct()">
                            Aceptar
                        </b-button>
                        <b-button size="sm" variant="danger" @click="clearProductsForm()">
                            Cancelar
            ...
            <div class="d-flex justify-content-between ml-0 mr-0 pl-0 pr-0">
                ...
                <b-button v-b-modal.modal-add-product-1 variant="primary" class="mb-3">Añadir producto</b-button>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            createProduct () {
                const payload = { id_product: this.idProduct, name: this.name, quantity: this.quantity, unit_price: this.unitPrice,
                    description: this.description,rut_user: this.rutUser}
                ...
                .catch(err => {
                    if (err.message === 'Request failed with status code 422') {
                        this.$swal('Algo malió sal :(', 'El producto ya está registrado en el sistema', 'error')
                    }
                })
            },
